...
export function PublicForm({ form, fields }: PublicFormProps) {
  ...
  const renderField = (field: FormField) => {
    const commonProps = {
      ...
      className:
        "w-full px-3 py-2 bg-white rounded-md text-muted-foreground focus:outline-none focus:ring-1 focus:ring-foreground border border-border",
  ...
  return (
    <div className="max-w-2xl mx-auto bg-white p-6 rounded-lg border border-border">
      <h1 className="text-xl font-medium text-foreground mb-6">{form.title}</h1>
      {submitStatus === "success" ? (
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            disabled={isSubmitting}
            className="px-4 py-2 bg-foreground text-white rounded-md hover:opacity-90 transition-colors disabled:opacity-50"
          >
