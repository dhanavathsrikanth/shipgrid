...
export function ContentModeration() {
  ...
  const renderItem = (item: ModeratableItem) => {
    ...
    return (
      <div
        key={item._id}
        className={`border-b border-muted py-4 last:border-b-0 transition-colors ${
        ...
        <div className="flex flex-col sm:flex-row items-start justify-between gap-4">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              ...
              {item.type === "story" && (
                <Link
                  href={`/s/${item.slug}`}
                  target="_blank"
                  className="font-medium text-muted-foreground hover:text-foreground block truncate"
                >
                  {item.title}
                </Link>
              )}
            ...
            {item.type === "story" && editingStoryId === item._id ? (
              <div className="mt-3 space-y-4 p-4 bg-muted rounded-lg border">
                {/* Basic Information */}
                <div>
              ...
              <p
                className={`text-sm ${item.type === "comment" ? "text-muted-foreground" : "text-muted-foreground"} mt-1 break-words`}
              >
            ...
            {item.type === "story" && item.customMessage && (
              <div className="mt-2 text-sm text-[#ffffff] bg-foreground border border-border rounded-md p-2 italic">
    ...
    <>
      ...
      <div className="space-y-6">
        <div className="bg-white rounded-lg p-4 sm:p-6 border border-gray-200">
            ...
            {activeItemType === "submissions" && (
              <div className="relative tag-search-container">
                ...
                {showTagDropdown && availableTags && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-border rounded-md max-h-48 overflow-y-auto">
                    {(() => {
          ...
          {activeItemType === "submissions" && (
            <div className="mb-6 p-4 bg-muted rounded-lg border border-gray-200">
          ...
          {uiIsLoading && (
            <div className="text-center py-6 text-lg font-medium text-muted-foreground">
              Loading...
            </div>
          )}
          ...
          {!uiIsLoading && itemsToRender.length > 0 && (
            <div>
              ...
              {activeItemType === "submissions" && itemsToRender.length > 0 && (
                <div className="border-b border-muted py-3 mb-2 bg-gray-50">
