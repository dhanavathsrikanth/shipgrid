    ...
    <div className="space-y-6">
      <div className="bg-white rounded-lg p-6 border border-gray-200">
        ...
        <div className="flex flex-wrap justify-between items-center mb-6 gap-4">
          <h2 className="text-xl font-medium text-muted-foreground">Manage Tags</h2>
          {hasPendingChanges && (
            <button
              onClick={handleSave}
              disabled={isProcessing}
              className="px-4 py-2 bg-muted text-muted-foreground rounded-md hover:bg-[#e5e1de] transition-colors flex items-center gap-2 disabled:opacity-50 text-sm"
            >
        ...
        <div className="mb-4">
          <input
            ...
            className="w-full px-3 py-2 border border-border rounded-md text-muted-foreground focus:outline-none focus:ring-1 focus:ring-foreground focus:border-foreground text-sm"
            disabled={isProcessing}
          />
        </div>
        ...
        <form onSubmit={handleAddTag} className="flex gap-2 mb-6">
          <input
            ...
            className="flex-1 px-3 py-2 border border-border rounded-md text-muted-foreground focus:outline-none focus:ring-1 focus:ring-foreground focus:border-foreground text-sm"
            disabled={isProcessing}
          />
          <button
            type="submit"
            disabled={!newTagName.trim() || isProcessing}
            className="px-4 py-2 bg-muted text-muted-foreground rounded-md hover:bg-[#e5e1de] transition-colors flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed text-sm"
          >
                    ...
                    {!tag.isDeleted ? (
                      <>
                        ...
                        <button
                          ...
                          className="text-muted-foreground hover:text-foreground disabled:opacity-50 p-1 transition-colors"
                        ...
                        <button
                          ...
                          className="text-destructive hover:text-destructive/80 disabled:opacity-50 p-1 transition-colors"
                          title="Delete tag"
                          disabled={isProcessing}
                        >
                      ...
                      <button
                        ...
                        className="text-xs text-muted-foreground hover:text-foreground font-medium disabled:opacity-50 p-1 transition-colors"
                        disabled={isProcessing}
                      >
                        Undo Delete
                      </button>
                  ...
                  <div className="p-3 border-t border-border bg-muted/30 space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <label
                        ...
                        className="text-xs font-medium text-muted-foreground w-20 flex-shrink-0"
                      >
                        BG Color:
                      </label>
                      <input
                        type="color"
                        id={`bg-color-${tag._id}`}
                        value={tag.backgroundColor || DEFAULT_TAG_BG}
                        onChange={(e) =>
                          handleFieldChange(
                            tag._id,
                            "backgroundColor",
                        ...
                        className="h-6 w-10 border border-border rounded cursor-pointer p-0.5 bg-clip-content"
                        disabled={isProcessing}
                      />
                      <input
                        type="text"
                        ...
                        className="px-2 py-1 text-xs border border-border rounded w-20 bg-background text-foreground"
                        disabled={isProcessing}
                      />
                      <button
                        onClick={() =>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        value={tag.textColor || DEFAULT_TAG_TEXT}
                        onChange={(e) =>
                          handleFieldChange(
                            tag._id,
                            "textColor",
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        value={tag.borderColor || DEFAULT_TAG_BORDER}
                        onChange={(e) =>
                          handleFieldChange(
                            tag._id,
                            "borderColor",
                  ...
                  <div className="p-3 border-t border-border bg-muted/30 space-y-3">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <input
                        ...
                        className="text-xs file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-xs file:bg-secondary file:text-secondary-foreground hover:file:bg-secondary/80 file:cursor-pointer"
                        ref={fileInputRef}
                        disabled={isProcessing}
                      />
                      <button
        ...
        <div className="mt-8 text-xs text-muted-foreground border-t border-border pt-6">
          <p className="font-medium mb-2">Help & Legend:</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <ul className="list-disc list-inside space-y-1">
              ...
              <li>
                <Trash2 className="w-3 h-3 inline mr-1 text-destructive" />: Mark for deletion.
              </li>
            </ul>
          </div>
          <div className="mt-4 flex flex-wrap gap-3 items-center">
            <span className="font-medium">Indicators:</span>
            <span className="px-1.5 py-0.5 border border-primary/30 bg-primary/5 text-primary rounded text-[10px]">
              New
            </span>
            <span className="px-1.5 py-0.5 border border-blue-300 bg-blue-50 text-blue-700 rounded text-[10px]">
              Modified
            </span>
            <span className="px-1.5 py-0.5 border border-destructive/30 bg-destructive/5 text-destructive rounded text-[10px]">
              Deleted
            </span>
      ...
      {showScrollButtons && (
        <div className="fixed bottom-6 right-6 flex flex-col gap-2 z-40">
          <button
            onClick={scrollToTop}
            className="p-3 bg-primary text-primary-foreground rounded-full hover:bg-primary/90 transition-all shadow-lg"
            title="Scroll to top"
          >
