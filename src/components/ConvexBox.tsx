...
export function ConvexBox() {
  ...
  return (
    <div
      className="fixed right-4 bottom-4 bg-card border border-border rounded-lg p-4 shadow-sm flex flex-col justify-center items-center text-center overflow-hidden"
      style={{
        width: "350px",
      ...
      {config.textAboveLogo ? (
        <>
          <a
            ...
            className="text-sm text-link hover:text-foreground font-medium break-words mb-2">
            {config.displayText}
          </a>
          {config.logoUrl && (
            <a
        ...
        <>
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="text-sm text-link hover:text-foreground font-medium break-words">
            {config.displayText}
          </a>
        </>
      )}
