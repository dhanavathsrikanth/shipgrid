...
export async function getAuthenticatedUserId(
  ...
  if (!user) {
    ...
    throw new ConvexError(
      "Authenticated user not found in Convex database. Please ensure you are logged in correctly.",
    );
  }
  return user._id;
}
...
export async function isUserAdmin(
  ...
  try {
    ...
    // Fallback: Check role directly from the user document in the database
    const user = await ctx.db
      .query("users")
      .withIndex("by_clerk_id", (q) => q.eq("clerkId", identity.subject))
      .first();
...
export async function requireAdminRole(
  ...
  console.error(
    "[requireAdminRole] 'admin' role NOT FOUND in Clerk token (top-level) or database. Access DENIED. Current role value in token:",
    clerkTokenRole,
  );
  throw new Error(
    "Admin privileges required. Role 'admin' not found in Clerk token or user database.",
  );
}
...
/**
 * Updates the currently authenticated user's ICP profile data.
 */
export const updateIcpProfile = mutation({
  args: {
    primaryProblem: v.string(),
...
export const setUsername = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // 2. Basic validation for username
