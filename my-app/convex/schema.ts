...
export default defineSchema({
  users: defineTable({
    ...
    imageUrl: v.optional(v.union(v.string(), v.null())), // Publicly visible profile image URL
    bio: v.optional(v.union(v.string(), v.null())), // User bio, max 200 chars (enforced in code)
    website: v.optional(v.union(v.string(), v.null())), // User website URL
    twitter: v.optional(v.union(v.string(), v.null())), // Twitter profile URL
    bluesky: v.optional(v.union(v.string(), v.null())), // Bluesky profile URL
    linkedin: v.optional(v.union(v.string(), v.null())), // LinkedIn profile URL
    ...
    emojiTheme: v.optional(v.union(v.string(), v.null())), // Emoji color theme preference
    // ICP Matching Fields
  ...
  settings: defineTable({
    ...
    // Platform Visibility & ICP Toggles
    showHackathon: v.optional(v.boolean()),
    showJudging: v.optional(v.boolean()),
    enableIcpMatching: v.optional(v.boolean()),
    showFounderAnalytics: v.optional(v.boolean()),
  }),
  ...
  newsletterSubscriptions: defineTable({
    ...
    userId: v.optional(v.id("users")), // Link to existing user if they are logged in
  ...
  productViews: defineTable({
    ...
    .index("by_story_viewed", ["storyId", "viewedAt"])
