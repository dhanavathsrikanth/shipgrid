...
// Query to get stories matched to the current user's ICP profile
export const listMatchedStories = query({
  args: {},
  handler: async (ctx) => {
    const identity = await ctx.auth.getUserIdentity();
    if (!identity) return [];
    ...
    if (!user || !user.icpComplete) return [];
    ...
    const scoredStories = await Promise.all(
      stories.map(async (story) => {
        ...
        // 2. Problem Match (30 pts)
        if (user.primaryProblem && story.icpProblem === user.primaryProblem) {
          score += 30;
        }

        // 3. Budget Match (20 pts)
        if (user.budgetRange && story.icpBudget === user.budgetRange) {
          score += 20;
        }
...
/**
 * Logs a view for a story with ICP matching data.
 */
export const logView = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert("productViews", {
      ...
      viewerProblem: user?.primaryProblem,
      viewerBudget: user?.budgetRange,
      isIcpMatch,
      viewedAt: Date.now(),
    });
  },
...
/**
 * Gets analytics for a founder's story.
 */
export const getMyProductAnalytics = query({
  args: { storyId: v.id("stories") },
  handler: async (ctx, args) => {
    const userId = await getAuthenticatedUserId(ctx);
    const story = await ctx.db.get(args.storyId);
    ...
    // Aggregate by role, problem, and budget
    ...
    views.forEach((v) => {
      if (v.viewerRole) {
        roleBreakdown[v.viewerRole] = (roleBreakdown[v.viewerRole] || 0) + 1;
      }
      if (v.viewerProblem) {
        problemBreakdown[v.viewerProblem] =
          (problemBreakdown[v.viewerProblem] || 0) + 1;
      }
      if (v.viewerBudget) {
        budgetBreakdown[v.viewerBudget] =
          (budgetBreakdown[v.viewerBudget] || 0) + 1;
      }
    });
...
/**
 * Gets aggregated analytics for all stories owned by the current user.
 */
export const getAllMyProductsAnalytics = query({
  args: {},
  handler: async (ctx) => {
    const userId = await getAuthenticatedUserId(ctx);
    ...
    const results = await Promise.all(
      stories.map(async (story) => {
        const views = await ctx.db
          .query("productViews")
          .withIndex("by_story_viewed", (q) => q.eq("storyId", story._id))
          .collect();
    ...
    const grandTotalViews = results.reduce((sum, r) => sum + r.totalViews, 0);
    const grandIcpViews = results.reduce((sum, r) => sum + r.icpViews, 0);
