...
const DEFAULT_SETTINGS = {
  ...
  showHackathon: true,
  showJudging: true,
  enableIcpMatching: true,
  showFounderAnalytics: true,
};
...
export const get = query({
  ...
  handler: async (ctx): Promise<SettingsData> => {
    ...
    return {
      ...
      showHackathon: settingsDoc.showHackathon ?? DEFAULT_SETTINGS.showHackathon,
      showJudging: settingsDoc.showJudging ?? DEFAULT_SETTINGS.showJudging,
      enableIcpMatching:
        settingsDoc.enableIcpMatching ?? DEFAULT_SETTINGS.enableIcpMatching,
      showFounderAnalytics:
        settingsDoc.showFounderAnalytics ??
        DEFAULT_SETTINGS.showFounderAnalytics,
    } as SettingsData; // Assert to ensure type compatibility
  },
});

// Development-only initialization (bypasses admin requirement)
export const initializeDev = internalMutation({
  args: {},
  handler: async (ctx): Promise<Id<"settings"> | null> => {
    ...
    console.log("Site settings initialized (development mode).");
    return settingsId;
  },
});
