...
export function SearchResults({ query, stories, viewMode }: SearchResultsProps) {
  ...
  return (
    <div>
      ...
      <div className="mb-6">
        <h2 className="text-xl text-muted-foreground mb-2">Search Results for "{query}"</h2>
      ...
      {users && users.length > 0 && (
        <div className="mb-8">
          <h3 className="text-lg font-medium text-foreground mb-4">Users</h3>
          <div className="grid gap-3">
            {users.map((user) => (
              <Link
                key={user._id}
                href={`/${user.username}`}
                className="flex items-center gap-3 p-3 rounded-md border border-border bg-white hover:bg-muted transition-colors"
              >
                {user.imageUrl ? (
                  <img
                    src={user.imageUrl}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {user.isVerified && (
                      <span className="text-xs text-foreground" title="Verified">
                        ✓
                      </span>
                    )}
                  </div>
      ...
      {stories.length > 0 && (
        <div>
          {users && users.length > 0 && (
            <h3 className="text-lg font-medium text-foreground mb-4">Apps</h3>
          )}
          <StoryList
            stories={stories}
            viewMode={viewMode}
