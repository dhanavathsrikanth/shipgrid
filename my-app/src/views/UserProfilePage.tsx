...
const Loading = () => <div className="text-center p-8 text-muted-foreground italic">Loading profile...</div>;
const ErrorDisplay = ({ message }: { message: string }) => (
  <div className="text-center p-8 text-destructive border border-destructive/20 bg-destructive/10 rounded-lg">Error: {message}</div>
);
...
export default function UserProfilePage({ username }: { username: string }) {
  ...
  // Analytics and Settings for Founder Dashboard
  const settings = useQuery(api.settings.get);
  const allAnalytics = useQuery(
    api.stories.getAllMyProductsAnalytics,
    isOwnProfileCandidate && settings?.showFounderAnalytics ? {} : "skip"
  );
  ...
  if (pathname.toLowerCase().startsWith("/user-settings")) {
    // Modified condition
    ...
    return (
      <div className="max-w-5xl mx-auto p-4 sm:p-6 bg-card rounded-md border border-border shadow-sm min-h-[600px] mt-8">
        <button
          onClick={() => router.back()}
          className="text-sm text-muted-foreground hover:text-foreground inline-block mb-6 bg-transparent border-none cursor-pointer p-0 transition-colors"
        >
  ...
  const ProfileImagePlaceholder = ({
    ...
    <div
      className={`rounded-full bg-muted flex items-center justify-center text-muted-foreground text-4xl font-bold border-2 border-border ${size}`}
    >
  ...
  return (
    <>
      <DialogComponents />
      {isOwnProfile && <IcpBanner />}
      <div className="max-w-4xl mx-auto p-4 sm:p-6 bg-background min-h-screen">
        <header
          className="mb-4 p-6 bg-card rounded-md border border-border shadow-sm font-sans"
        >
            ...
            <div className="relative mb-4 sm:mb-0 sm:mr-6 rounded-full w-24 h-24 overflow-hidden">
              {isEditing ? (
                ...
                >
                  {newProfileImagePreview ? (
                    <img
                      ...
                      alt="Profile preview"
                      className="w-full h-full object-cover border-4 border-muted group-hover:opacity-75 w-24 h-24 overflow-hidden rounded-full transition-opacity shadow-inner"
                    />
                  ) : (
                ...
                <img
                  ...
                  className="rounded-full h-19 object-cover border-2 border-border shadow-sm"
                />
              ) : (
                <ProfileImagePlaceholder
                  name={loadedProfileUser?.name}
            ...
            <div className="flex-grow text-left sm:text-left">
              {isEditing ? (
                <div className="space-y-2 mb-2">
                  ...
                  <input
                    ...
                    className="text-xl font-normal text-foreground w-full px-2 py-1 border border-gray-300 rounded-md text-sm focus:outline-none focus:border-black"
                ...
                <div className="flex items-baseline mb-1">
                  <h1
                    className="text-lg font-normal text-foreground mr-2 font-sans"
                  >
                  ...
                  <p
                    className="text-lg text-muted-foreground font-sans outline-none"
                  >
                    {/* @{loadedProfileUser?.username || "N/A"}{" "} */}
                    {typeof userNumber === "number" && (
                      <span className="ml-0 text-xs text-muted-foreground/60">
                        User #{userNumber}
                      </span>
                    )}
                  </p>
              ...
              <div className="mb-3 w-full text-left">
                {isEditing ? (
                  <textarea
                    ...
                    maxLength={200}
                    className="w-full px-2 py-1 border border-border bg-background rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-ring transition-all font-sans"
                    placeholder="Add a short bio (max 200 chars)"
                    rows={3}
                  />
                ) : loadedProfileUser?.bio ? (
                  <p
                    className="text-sm text-muted-foreground w-full text-left font-sans"
                  >
                    {loadedProfileUser.bio}
                  </p>
                ) : (
                  <p
                    className="text-sm text-muted-foreground/50 italic w-full text-left font-sans"
                  >
                    No bio yet.
                  </p>
                )}
                ...
                {isEditing ? (
                  <>
                    <input
                      ...
                      className="flex-grow sm:w-auto px-2 py-1 border border-border bg-background rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-ring transition-all"
                      placeholder="Website"
                    />
                    <input
                      type="url"
              ...
              {isOwnProfile && !isEditing && (
                <div className="mt-3 flex flex-col md:flex-row md:items-center gap-2 md:gap-4">
                  ...
                  <button
                    onClick={() =>
                      ownInboxEnabled !== false && router.push("/inbox")
                    }
                    disabled={ownInboxEnabled === false}
                    className={`px-6 py-2 rounded-md border border-border text-sm font-medium flex items-center justify-center transition-colors ${
                  ...
                  <button
                    ...
                    className="px-6 py-2 rounded-md bg-primary border border-border text-primary-foreground text-sm font-medium hover:bg-primary/90 flex items-center justify-center transition-colors"
                  >
          ...
          {isEditing && (
            <div className="mt-6 pt-4 border-t border-gray-200 flex flex-col sm:flex-row justify-end space-y-2 sm:space-y-0 sm:space-x-3">
              ...
              <button
                ...
                disabled={isSaving}
                className="px-4 py-2 bg-secondary text-secondary-foreground rounded-md hover:bg-secondary/80 transition-colors flex items-center justify-center"
              >
              ...
              <button
                ...
                className="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90 transition-colors flex items-center justify-center disabled:opacity-50"
              >
        ...
        <section
          className="mb-4 p-4 rounded-md border bg-card border-border shadow-sm font-sans"
        >
          <h2 className="text-lg font-normal text-foreground mb-4 pb-2 border-b border-border">
            My Vibes
          ...
          <div className="flex flex-col gap-2 md:flex-row md:flex-wrap md:gap-3 md:justify-start">
            ...
            <button
              ...
              className="flex items-center p-3 bg-card border border-border rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
            >
              <BookOpen className="w-6 h-6 mr-3 text-muted-foreground md:w-8 md:h-8 md:mb-2 md:mr-0" />
              <div className="flex flex-col md:items-center">
                <span className="text-xl font-bold text-foreground">
                  {stories.length}
                </span>
                <span className="text-xs text-muted-foreground md:mt-0.5">
                  Submissions
                </span>
              </div>
            </button>
            ...
            <button
              ...
              className="flex items-center p-3 bg-card border border-border rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
            >
              <ThumbsUp className="w-6 h-6 mr-3 text-muted-foreground md:w-8 md:h-8 md:mb-2 md:mr-0" />
              <div className="flex flex-col md:items-center">
                <span className="text-xl font-bold text-foreground">
                  {votes.length}
                </span>
                <span className="text-xs text-muted-foreground md:mt-0.5">Votes</span>
              </div>
            </button>
            ...
            <button
              ...
              className="flex items-center p-3 bg-card border border-border rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
            >
              <Star className="w-6 h-6 mr-3 text-muted-foreground md:w-8 md:h-8 md:mb-2 md:mr-0" />
              <div className="flex flex-col md:items-center">
                <span className="text-xl font-bold text-foreground">
                  {ratings.length}
                </span>
                <span className="text-xs text-muted-foreground md:mt-0.5">Ratings</span>
              </div>
            </button>
            ...
            <button
              ...
              className="flex items-center p-3 bg-card border border-border rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
            >
              <MessageCircle className="w-6 h-6 mr-3 text-muted-foreground md:w-8 md:h-8 md:mb-2 md:mr-0" />
              <div className="flex flex-col md:items-center">
                <span className="text-xl font-bold text-foreground">
                  {comments.length}
                </span>
            ...
            {isOwnProfile && (
              <button
                ...
                className="flex items-center p-3 bg-card border border-border rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
              >
                <Bookmark className="w-6 h-6 mr-3 text-muted-foreground md:w-8 md:h-8 md:mb-2 md:mr-0" />
                <div className="flex flex-col md:items-center">
                  <span className="text-xl font-bold text-foreground">
            ...
            <button
              ...
              className="flex items-center p-3 bg-card border border-border rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
            >
              <Users className="w-6 h-6 mr-3 text-muted-foreground md:w-8 md:h-8 md:mb-2 md:mr-0" />
              <div className="flex flex-col md:items-center">
                <span className="text-xl font-bold text-foreground">
                  {followersCount ?? 0}
                </span>
            ...
            <button
              ...
              className="flex items-center p-3 bg-card border border-border rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
            >
              <UserPlus className="w-6 h-6 mr-3 text-muted-foreground md:w-8 md:h-8 md:mb-2 md:mr-0" />
              <div className="flex flex-col md:items-center">
                <span className="text-xl font-bold text-foreground">
                  {followingCount ?? 0}
                </span>
            ...
            {/* Founder Insights (Own Profile Only and if enabled) */}
            {isOwnProfile && settings?.showFounderAnalytics && (
              <button
                onClick={() => handleMiniDashboardClick("analytics")}
                aria-label="View your founder insights"
                className="flex items-center p-3 bg-primary/5 border border-primary/20 rounded-lg shadow-sm w-full h-auto justify-start hover:transform hover:-translate-y-1 hover:shadow-lg transition-all duration-200 ease-in-out cursor-pointer focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary md:flex-col md:text-center md:w-24 md:flex-shrink-0 md:h-24 md:justify-center md:p-4"
              >
                < BarChart3 className="w-6 h-6 mr-3 text-primary md:w-8 md:h-8 md:mb-2 md:mr-0" />
                <div className="flex flex-col md:items-center">
                  <span className="text-xl font-bold text-primary">
                    {allAnalytics?.grandTotalViews ?? 0}
                  </span>
                  <span className="text-xs text-primary/80 md:mt-0.5">
                    Insights
                  </span>
                </div>
              </button>
        ...
        <section
          ...
          id="submissions"
          className="mb-6 p-4 bg-card rounded-md border border-border shadow-sm"
        >
          <h2 className="text-lg font-normal text-foreground mb-4 pb-2 border-b border-border">
            Submissions
          </h2>
                ...
                >
                  ...
                  {isOwnProfile && (
                    <div className="flex items-center gap-2">
                      <Link
                        href={`/s/${story.slug}?edit=true`}
                        className="text-sm text-primary hover:text-primary/80 hover:bg-primary/10 p-2 rounded-md flex items-center gap-1 flex-shrink-0 transition-colors"
                      >
                        <Edit3 className="w-4 h-4" /> Edit
                      </Link>
                      <button
                        onClick={() => handleDeleteStory(story._id)}
                        className="text-sm text-destructive hover:text-destructive/80 hover:bg-destructive/10 p-2 rounded-md flex items-center gap-1 flex-shrink-0 transition-colors"
                      >
        ...
        <div className="mb-20">
          {/* Tab Buttons */}
          <div className="flex flex-col gap-2 md:flex-row md:flex-wrap border-b border-border mb-4">
            <button
              onClick={() => setActiveTab("votes")}
              className={`w-full text-left md:w-auto py-2 px-4 text-sm font-medium focus:outline-none transition-all ${
                activeTab === "votes"
            ...
            {isOwnProfile && (
              <button
                ...
                className={`w-full text-left md:w-auto py-2 px-4 text-sm font-medium focus:outline-none flex items-center transition-all ${
                  activeTab === "bookmarks"
                    ? "border-b-2 border-primary text-foreground"
                    : "text-muted-foreground hover:text-foreground hover:border-border"
                }`}
          ...
          >
            ...
            {activeTab === "ratings" && (
              ...
              >
                {ratings.length === 0 && (
                  <p className="text-muted-foreground italic">No ratings given yet.</p>
                )}
                {ratings.length > 0 && (
                  <ul className="space-y-4">
                          ...
                          <p className="text-sm text-amber-500 flex items-center">
                            ...
                            {Array(5 - rating.value)
                              ...
                              .map((_, i) => (
                                <Star
                                  key={i + rating.value}
                                  className="w-4 h-4 text-muted border-none fill-current"
                                />
                              ))}
                ...
                {comments.length > 0 && (
                  <ul className="space-y-4">
                    {comments.map((comment: CommentInProfile) => (
                      ...
                      >
                        <div className="flex-grow mr-4">
                          <p className="text-foreground mb-1 whitespace-pre-wrap">
                            {comment.content}
                          </p>
                          <p className="text-xs text-muted-foreground/60">
                            Commented on{" "}
                            <Link
                              href={`/s/${comment.storySlug}#comments`}
            ...
            {activeTab === "bookmarks" && (
              <section
                ...
                className="p-4 rounded-md border bg-card border-border shadow-sm"
              >
            ...
            {activeTab === "followers" && (
              <section
                ...
                className="p-4 bg-card rounded-md border border-border shadow-sm"
              >
                {followersData === undefined && (
                  <p className="text-center p-8 text-muted-foreground">Loading followers...</p>
                )}
                {followersData && followersData.length === 0 && (
                  <p className="text-muted-foreground italic">No followers yet.</p>
                )}
                {followersData && followersData.length > 0 && (
                  <ul className="space-y-3">
                    {followersData.map((follower: FollowUserListItem | null) =>
                      follower ? (
                        <li
                          key={follower._id}
                          className="p-3 bg-muted/30 border border-border rounded-md flex items-center justify-between transition-all hover:bg-muted/50"
                        >
                          <Link
            ...
            {activeTab === "following" && (
              ...
              >
                {followingData === undefined && (
                  <p className="text-center p-8 text-muted-foreground">Loading following...</p>
                )}
                        ...
                        followedUser ? (
                          ...
                          >
                            ...
                            >
                              ...
                              <div>
                                <span className="text-sm p-1 font-semibold text-foreground hover:underline">
                ...
                {!allAnalytics ? (
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {/* Total Views Card */}
                      <div className="p-6 bg-card border border-border rounded-xl shadow-sm hover:border-primary/20 transition-colors">
                        ...
                        <p className="text-3xl font-bold text-foreground">{allAnalytics.grandTotalViews}</p>
                      </div>

                      {/* ICP Match Rate Card */}
                      <div className="p-6 bg-card border border-border rounded-xl shadow-sm hover:border-green-500/20 transition-colors">
                        <div className="flex items-center gap-4 mb-2">
                          <div className="p-2 bg-green-500/10 rounded-lg">
                            <Target className="w-5 h-5 text-green-500" />
                          </div>
                          <h3 className="text-sm font-medium text-muted-foreground">Match Rate</h3>
                        </div>
                        <p className="text-3xl font-bold text-foreground">
                          {allAnalytics.grandTotalViews > 0 
                            ? ((allAnalytics.grandIcpViews / allAnalytics.grandTotalViews) * 100).toFixed(1)
                            : "0.0"}%
                        </p>
                        <p className="text-xs text-muted-foreground mt-1">People from your target audience</p>
                      </div>

                      {/* ICP Views Card */}
                      <div className="p-6 bg-card border border-border rounded-xl shadow-sm hover:border-purple-500/20 transition-colors">
                        <div className="flex items-center gap-4 mb-2">
                          <div className="p-2 bg-purple-500/10 rounded-lg">
                            <PieChart className="w-5 h-5 text-purple-500" />
                          </div>
                          <h3 className="text-sm font-medium text-muted-foreground">ICP Impressions</h3>
                        </div>
                        <p className="text-3xl font-bold text-foreground">{allAnalytics.grandIcpViews}</p>
                      </div>
                    </div>
                    ...
                    <div className="p-6 bg-card border border-border rounded-xl shadow-sm">
                      ...
                      <div className="space-y-4">
                        {allAnalytics.stories.length === 0 && (
                          <div className="p-8 text-center border-2 border-dashed border-border rounded-lg text-muted-foreground italic">
                            No products found. Start by submitting a project!
                          </div>
                        )}
                        {allAnalytics.stories.map((s: any) => (
                          <div key={s.storyId} className="group flex flex-col sm:flex-row sm:items-center justify-between p-5 bg-muted/20 border border-border rounded-xl gap-4 hover:bg-muted/30 transition-all">
                            <div className="space-y-1">
                              <h4 className="font-semibold text-foreground group-hover:text-primary transition-colors">{s.storyTitle}</h4>
                              <p className="text-xs text-muted-foreground flex items-center gap-1.5">
                                <span className="w-2 h-2 rounded-full bg-border" />
                                {s.totalViews} total views
                              </p>
                            </div>
                            <div className="flex flex-col items-end gap-2">
                              <div className="text-sm font-bold text-primary flex items-center gap-1.5">
                                <Target className="w-4 h-4" />
                                {s.icpViews} ICP matches
                              </div>
                              <div className="relative w-40 h-2 bg-muted rounded-full overflow-hidden">
                                <div 
                                  ...
                                  style={{ width: `${s.totalViews > 0 ? (s.icpViews / s.totalViews * 100) : 0}%` }}
                                />
                              </div>
                              <span className="text-[10px] uppercase tracking-wider font-bold text-muted-foreground">
                                {s.totalViews > 0 ? ((s.icpViews / s.totalViews) * 100).toFixed(0) : 0}% TA MATCH
                              </span>
                            </div>
                          </div>
                        ))}
        ...
        {isOwnProfile && (
          <section
            ...
            className="mb-4 p-6 bg-card rounded-md border border-border shadow-sm font-sans"
          >
            <h2 className="text-lg font-normal text-foreground mb-6 pb-3 border-b border-border">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <button
                    ...
                    className="w-full px-4 py-2 text-left bg-muted/50 hover:bg-muted border border-border rounded-md text-sm text-foreground transition-colors disabled:opacity-50 flex items-center"
                  >
                ...
                <div>
                  ...
                  <Link
                    href="/user-settings"
                    className="w-full mt-2 px-4 py-2 text-left bg-muted/50 hover:bg-muted border border-border rounded-md text-sm text-foreground transition-colors flex items-center"
                  >
                  ...
                  <div className="mt-4 p-4 bg-background border border-border rounded-md">
                    ...
                    {emailSettingsData === undefined ? (
                      <div className="flex items-center gap-2">
                        <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-muted-foreground/30"></div>
        ...
        {!isOwnProfile && isClerkLoaded && authUser && loadedProfileUser && (
          <div className="mt-8 p-4 bg-muted/30 rounded-lg border border-border flex items-center justify-between text-sm text-muted-foreground">
            <div className="flex items-center gap-3">
              <Flag className="w-4 h-4 text-muted-foreground/60 flex-shrink-0" />
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <DialogFooter className="sm:justify-start">
              ...
              <Button
                ...
                className="bg-primary text-primary-foreground hover:bg-primary/90 disabled:opacity-50 sm:ml-[10px]"
              >
